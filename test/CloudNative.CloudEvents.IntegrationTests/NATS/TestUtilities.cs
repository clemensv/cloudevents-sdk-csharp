...
namespace IntegrationTests
{
    public class NATSServer : IDisposable
    {
        ...
        {
            // this depends on the current directory structure of this project
            string tmppath = Path.GetFullPath(Path.Combine(Assembly.GetExecutingAssembly().Location, @"..\..\..\..\..\..\tmp\"));
            string[] natspath = Directory.GetDirectories(tmppath, "nats-server-*");
            if ( natspath.Length == 0 )
            {
                throw new FileNotFoundException("Cannot find NATS server directory. Run test-setup.ps1 to intialize");
            }
            psInfo = new ProcessStartInfo(Path.Combine(natspath[0], SERVEREXE));
    ...
    public class UnitTestUtilities
    {
        ...
        {
            var configDirPath = Path.Combine(Environment.CurrentDirectory, @"NATS\config");
